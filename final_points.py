...
class final_points_lowest_full_replacement(final_points):
    ...
    def calculate_need_grd(self):
        self._need_grade = min(
            ((self._goal - self._current_grade - self._drop_test_score) / (self._final_worth + self._drop_test_worth)),
            ((self._goal_point - self._current_grade) / self._final_worth * 100))
        self._need_grade_point = self._goal_point - self._current_grade
